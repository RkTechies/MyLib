...
class ReadMoreOption (
    ...
    fun displayHtml( html: String,mTextView:TextView) {
        val imageGetter = ImageGetter(context!!.resources, mTextView)
    ...
    fun addReadMoreTo(textView: TextView, text: CharSequence) {
        if (textLengthType == ReadMoreOption.TYPE_CHARACTER) {
            if (text.length <= textLength) {
                displayHtml(text.toString(),textView);
                //textView.text = text
                return
            }
        } else {
        ...
        textView.post(Runnable {
            ...
            displayHtml(ss.toString(),textView);
            //textView.text = ss
